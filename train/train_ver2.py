...
def train(opt):
    ...
    while n_epi < start_episode + opt.num_episodes:
        ...
        for trajectory in all_trajectories:
            ...
            if trajectory_returns.shape[0] < 2048:
                trajectory_returns_len = trajectory_returns.shape[0]
                mask = torch.ones(2048, device=trajectory_returns.device)
                mask[trajectory_returns_len:] = torch.nan
                trajectory_returns = torch.cat((trajectory_returns, mask[trajectory_returns_len:]))
                
                
            inner_group_advantage_list.append(trajectory_returns)
        
        stacked_advantages = torch.stack(inner_group_advantage_list, dim=0)
        mean_advantages = torch.nanmean(stacked_advantages, dim=0)
        std_advantages = torch.sqrt(torch.nanmean((stacked_advantages - mean_advantages.unsqueeze(0))**2, dim=0))
        
        group_advantages = []
        for trajectory_returns in inner_group_advantage_list:
            mask = ~torch.isnan(trajectory_returns)
            normalized_returns = torch.zeros_like(trajectory_returns)
            normalized_returns[mask] = (trajectory_returns[mask] - mean_advantages[mask]) / (std_advantages[mask] + 1e-8)
            normalized_returns = torch.clamp(normalized_returns, -10.0, 10.0)
            group_advantages.append(normalized_returns[mask])
